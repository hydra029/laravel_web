...
@push('js')
	...
	<script>
        $(document).ready(function () {
            ...
            let tdTime       = new Date(new Date(new Date(td).setHours(7)).setDate(1)).getTime();
                            ...
                            for (let i = 0; i < emp_num; i++) {
                                ...
                                if (length === 0) {
                                    ...
                                    if (days < total_day) {
                                        ...
                                        let crD     = new Date(s).getDate();
                                        let crDate  = new Date(td).getDay();
                                        let crMonth = new Date(td).getMonth() + 1;
                                        let crYear  = new Date(td).getFullYear();
                                        let slMonth = sl2.children(':selected').val();
                                        let slYear  = sl1.children(':selected').val();
                                        if (crDate === 0) {
                                            crDate = 7;
                                        }
                                        total_day = 7 - days;
                                        if (crYear === parseInt(slYear)) {
                                            if (crMonth === parseInt(slMonth)) {
                                                if (days < crDate) {
                                                    total_day = 6 - crDate;
                                                }
                                            }
                                        }
                                        for (let u = total_day - 1; u >= 0; u--) {
                                            let d    = new Date(s).setDate(crD - u);
