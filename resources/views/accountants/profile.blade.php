		...
		<form id="information-form" method="post" enctype="multipart/form-data">
			<div class="profile-card col-12 table-profile-update form-row">
				<div class="image-upload">
					...
					<input id="avatar" type="file" name="avatar" class="disabled d-none">
				</div>
				<div class="profile-card-info float-left">
					...
					<div class="form-row">
						<div class="form-group col-md-6">
							...
							<input type="text" name="email" id="email" class="form-control inp-email disabled"
	...
	<script>
        $(document).ready(async function () {
            ...
            btnSave.click(function (e) {
                ...
                let data       = getFormData(formUpdate);
                data.city      = $('#select2-city-container').prop('title');
                data.district  = $('#select2-district-container').prop('title').replace(/\s{2,}/g, '');
                $.ajax({
                    type    : "post",
                    url     : "{{ route('accountants.update_information') }}",
                    dataType: 'json',
                    data    : data,
                })
