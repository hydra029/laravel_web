	...
	<div class="dept-list col-12">
		...
		<div class="col-12 p-2 border border-1 border-light dept-list">
			<table class="table table-striped table-striped" id="table-list-department">
				<thead>
				<th class="col-2 am"><span>#</span></th>
				<th class="col-2 am"><span>Name</span></th>
		...
		<div class="popup-edit-department d-none model-popup">
			...
			<form id="form-update-department" action="{{ route('ceo.department.update') }}" method="post"
				...
				<table class="table form-table">
					...
					<tr>
						<td>
							<button class="btn btn-primary btn-change-department float-right ml-1" type="submit">
								Submit
							</button>
							<button class="btn btn-light btn-close-model float-right" type="button">
								Cancel
							</button>
						</td>
					</tr>
	...
	<script type="text/javascript">
        $(document).ready(async function () {
                    ...
                    $.ajax({
                        ...
                        success : function (response) {
                            ...
                            if (response[0]['manager']['avatar'] === null) {
                                ...
                                img = '{{ asset('') }}' + 'img/' + response[0]['manager']['avatar'];
                            }
                            $('.model-popup-div ').addClass('d-none');
                            modelPopup.addClass('d-none');
                            $('.popup-edit-department').addClass('d-none');
                            dept_row.find('.dept-name').text(response[0]['name']);
                            dept_row.find('.manager-id').text(response[0]['manager']['id']);
                            dept_row.find('.manager-name').text(response[0]['manager']['full_name']);
                            dept_row.find('.manager-avatar-img').attr('src', img);
                            dept_row.find('.manager-gender').text(response[0]['manager']['gender_name']);
                            dept_row.find('.manager-dob').text(response[0]['manager']['date_of_birth']);
                            dept_row.find('.manager-email').text(response[0]['manager']['email']);
                            dept_row.find('.manager-phone').text(response[0]['manager']['phone']);
                            dept_row.find('.manager-role').text(response[0]['manager']['role']);
                            dept_row.find('.manager-address').text(response[0]['manager']['address']);
                            dept_row = '';
                        }
                    })
                });
            ...
            $('.btn-show-department').click(function () {
                ...
                let url          = "{{ route('ceo.department_employees') }}";
                ;
                if (dept_id === 1) {
                    ...
                    success : function (response) {
                        ...
                        $.each(response.data.data, function (index, value) {
                            ...
                            if (value['avatar'] == null) {
                                img = `<img src="{{ asset('img/istockphoto-1223671392-612x612.jpg') }}" style=" border-radius:50% " width="40px">`
                            }
                ...
                $('.btn-show-employee').click(function () {
                    ...
                    $.ajax({
                        ...
                        success : function (response) {
                            ...
                            profileCard.find('.profile-card-info')
            ...
            $.each(cities, function (index, each) {
                selectCity.append(`<option value='${each.code}' data-path='${each['file_path']}'>${index}</option>`);
            });
            ...
            $('.manager-name').click(function (e) {
                ...
                let profileInfo     = profileCard.find('.profile-card-info');
                let id_manager      = tr.find('.manager-id').text();
                let name_manager    = tr.find('.manager-name').text();
                let gender_manager  = tr.find('.manager-gender').text();
                let dob_manager     = tr.find('.manager-dob').text();
                let email_manager   = tr.find('.manager-email').text();
                let phone_manager   = tr.find('.manager-phone').text();
                let address_manager = tr.find('.manager-address').text();
                let id_role_manager = tr.find('.manager-role').text();
                let avatar_manager  = tr.find('.manager-avatar').text();
                profileInfo.find('.profile-card-name').text(name_manager);
                profileInfo.find('.profile-card-gender').text(gender_manager);
                profileInfo.find('.profile-card-dob').text(dob_manager);
                profileInfo.find('.profile-card-email').text(email_manager);
                profileInfo.find('.profile-card-number-phone').text(phone_manager);
                profileInfo.find('.profile-card-address').text(address_manager);
                if (avatar_manager === '') {
